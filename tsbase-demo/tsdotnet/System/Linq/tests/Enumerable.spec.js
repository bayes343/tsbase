...
describe('Enumerable', () => {
    ...
    it('should take items while a condition is met', () => {
        ...
        expect(noMatches.Count).toEqual(0);
        const first4 = classUnderTest.TakeWhile(item => item.value < 5);
        expect(first4.Count).toEqual(4);
    });
    it('should return distinct elements from the enumerable collection', () => {
        ...
        expect(distinctElements.Count).toEqual(4);
    });
    ...
    it('should skip items while a condition is true then return the rest', () => {
        ...
        expect(oneItem.Count).toEqual(1);
    });
    it('should aggregate the elements based on input parameter accumulator and result functions', () => {
        ...
        const three = classUnderTest.Aggregate(1, (largest, next) => largest > next ? largest : next, item => item.toString());
        expect(three).toEqual('3');
        // simple aggregation
        const eleven = classUnderTest.Aggregate(0, (current, next) => current + next, item => item.toString());
